...
export async function getQueueItemById(id) {
  const db = await getDatabase();
  const row = await db.getFirstAsync('SELECT * FROM capture_queue WHERE id = ?', [id]);
  if (!row) return null;
  return {
    ...row,
...
export async function clearCompletedQueueItems() {
  const db = await getDatabase();
  const rows = await db.getAllAsync("SELECT id FROM capture_queue WHERE status = 'uploaded'");
  for (const row of rows) {
    const itemDir = `${FileSystem.documentDirectory}captured_queue/${row.id}/`;
    await FileSystem.deleteAsync(itemDir, { idempotent: true }).catch(() => {});
  }
  await db.runAsync("DELETE FROM capture_queue WHERE status = 'uploaded'");
}

export async function clearAllQueueItems() {
  const db = await getDatabase();
  const baseDir = `${FileSystem.documentDirectory}captured_queue/`;
  await FileSystem.deleteAsync(baseDir, { idempotent: true }).catch(() => {});
